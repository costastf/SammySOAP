...
from network import Network # we use the network helper class to get basic info about local interface
    ...
    def sendKeys(self, keys):
        ...
        if not isinstance(keys,(list, tuple)):
            ...
            for key in keys:
                if key != 'KEY_DNIe':
                    key = key.upper()
                if key not in self.supportedKeys:
                    safeKeys = False
                    break
        if safeKeys:        
            for key in keys:    
...
if __name__=='__main__':
    ...
    # or 
    remote.sendKeys(("KEY_1", "KEY_ENTER"))
    time.sleep(5)
